regius.controller('emprestimoCtrl', function ($scope, $state, $ionicPopup, apiService) {

    $scope.listaEmprestimos = [];
    $scope.possuiInforme = false;

    var url = '/plan/:id/emprestimos/'.replace(':id', window.localStorage.plan_id);
    var method = 'get';
    ...
    $scope.detalhar = function (emprestimo) {
        $state.go('emprestimo-detalhe', { ano: emprestimo.ano, numero: emprestimo.numero });
    }
    ...
    $scope.simular = function () {
        ...
        else $ionicPopup.show({
                title: 'Ops',
                subTitle: "Já existe um contrato na situação 'Em deferimento' e o sistema não permite novos empréstimos.",
                buttons: [
                  { text: 'Ok' }
                ]
            });
