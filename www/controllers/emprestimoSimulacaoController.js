regius.controller(
  "emprestimoSimulacaoCtrl",
  function ($scope, $state, $q, apiService, $ionicPopup) {
    $scope.dados = {};
    ...
    var url = "/plan/:id/emprestimos/dadosSimulacao".replace(
      ":id",
    ...
    $scope.calcularMargem = function () {
      if ($scope.dados.data) {
        ...
        for (var i = 0; i < $scope.dados.data.contratos.length; i++) {
          ...
          console.log({
            valor,
            valorPrestacao: $scope.dados.data.contratos[i].valorPrestacao,
          });
        }
      ...
      if (!$scope.dados.valorSolicitado) {
        ...
        for (var i = 0; i < $scope.dados.data.contratos.length; i++) {
          if ($scope.dados.data.contratos[i].checked) {
            ...
            if (i + 1 < $scope.dados.data.contratos.length) contratos += ",";
          }
        }

        var url = "/plan/:id/emprestimos/validaSolicitacao".replace(
          ":id",
          window.localStorage.plan_id
        );
        var method = "post";
        ...
        apiService.list(url, method, data, function (result) {
          localStorage.setItem(
            ...
            JSON.stringify($scope.dados.modalidade)
          );
          localStorage.setItem(
            "emprestimo-data",
            JSON.stringify($scope.dados.data)
          );
          localStorage.setItem(
            "emprestimo-valorSolicitado",
            $scope.dados.valorSolicitado
          );
