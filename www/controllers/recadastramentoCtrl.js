  ...
  .controller(
    ...
    function (
      ...
      $scope.$on("$ionicView.enter", function (scope, states) {
        if (states.stateName == "recadastramento") $scope.init();
      });
        ...
        apiService.listData($scope, url, method, null, function (data) {
          if (data != "") {
            ...
            if (passoArmazenado && passoArmazenado.toString() != "null") {
              ...
              $scope.oidSolicitacao = parseInt(
                localStorage.getItem("recadastramentoSolicitacao")
              );
              buscarPasso();
            }
          }
      ...
      function validarCampos() {
        ...
        for (
          i = 0, count = $scope.recadastramento.GrupoCampos.length;
          i < count;
          i++
        ) {
          var grupoCampos = $scope.recadastramento.GrupoCampos[i];
          ...
          if (grupoCampos.ExigeComprovacao) {
            for (
              i2 = 0, count2 = grupoCampos.Campos.length;
              i2 < count2;
              i2++
            ) {
              var campo = grupoCampos.Campos[i2];
              ...
              var valor =
                campo.TipoCampo == "TXT" || campo.TipoCampo == "EMA"
                  ? campo.NovoValor
                  : campo.NovoValorCombo;
            ...
            if (!camposValidos)
              $scope.showAlert(
                ...
                "O grupo " +
                  grupoCampos.Titulo +
                  " foi alterado e necessita de comprovação."
              );
          } else {
            for (
              i2 = 0, count2 = grupoCampos.Campos.length;
              ...
              if (
                campo.ExigeComprovacao &&
                campo.ValorAntigo != valor &&
                campo.Arquivo == null
              ) {
                camposValidos = false;
                $scope.showAlert(
            ...
            if (
              ...
              window.FilePath.resolveNativePath(imagePath, function (entry) {
                ...
                function success(fileEntry) {
                  var namePath = fileEntry.nativeURL.substr(
                    0,
                    fileEntry.nativeURL.lastIndexOf("/") + 1
                  );
      ...
      $scope.uploadImage = function (campo, isGrupo) {
        ...
        $cordovaFileTransfer
          ...
          .then(function (result) {
            ...
            if (isGrupo) {
              campo.Campos[0].Arquivo = retorno.filename;
